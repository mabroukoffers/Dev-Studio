...
export const PRAYER_META: Record<string, {
  arabic: string; Icon: LucideIcon; color: string; iconColor: string;
}> = {
  ...
  Maghrib: { arabic: "المغرب", Icon: Sunset,  color: "text-orange-500",  iconColor: "text-orange-500"  },
  Isha:    { arabic: "العشاء", Icon: MoonStar,color: "text-violet-400",  iconColor: "text-violet-400"  },
};
...
export function usePrayerTimes(date?: string): UsePrayerTimesResult {
  ...
  useEffect(() => { geolocate(); }, [targetDate]);
